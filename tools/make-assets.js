...
//   表紙は src/app.html の1曲目の表紙を使う。
//   表紙と同じ場所に「〜-icon.jpg」（例 assets/covers/01-icon.jpg）があれば、アイコンはそちらから作る。
//   無ければ表紙の真ん中を正方形に切る。COVER_Y=35% のように縦位置を変えられる。
...
(async () => {
  ...
  const iconFile = cover.replace(/\.(\w+)$/, '-icon.$1');
  const icon = fs.existsSync(path.join(root, iconFile)) ? iconFile : cover;
  const b = await chromium.launch();
  const p = await b.newPage({ viewport: { width: 1300, height: 1800 } });
  await p.goto('file://' + path.join(__dirname, 'card.html') + '?cover=' + encodeURIComponent('../' + cover) + '&icon=' + encodeURIComponent('../' + icon) + '&y=' + encodeURIComponent(y));
  ...
  await b.close();
  console.log('書き出しました（表紙:', cover, '／ アイコン:', icon + '）');
})();
